...
/** Base product name (before " - " if variation), else full name */
function getBaseName(item: CartItem): string {
  const idx = item.name.indexOf(" - ")
  return idx >= 0 ? item.name.slice(0, idx).trim() : item.name.trim()
}

/** Quantity + unit for display: "85 × 50kg" for variation, "8 kg" for base */
function getQuantityUnitDisplay(item: CartItem): string {
  const v = item.variation

  // Prefer explicit variation quantity/unit if present (prevents "25 25kg" duplication).
  let pack = ""
  if (v?.quantity != null) {
  ...
  // If pack looks like a size (contains a number, e.g. "50kg"), use ×. Otherwise use plain unit.
              ...
              {items.map(item => (
                <div key={item.id} className="flex items-start space-x-3 p-3 bg-gray-50 rounded-lg">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-gray-900 truncate">{getBaseName(item)}</p>
