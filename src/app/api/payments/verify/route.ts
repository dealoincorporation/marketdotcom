...
import { applyReferralBonusOnFirstPurchase } from "@/lib/referralBonus"
    ...
    if (paymentStatus === 'COMPLETED') {
      try {
        ...
        if (fullOrder && fullOrder.user) {
          ...
          // Referral bonus: both referrer and referee get ₦500 after referee's first purchase
          try {
            await applyReferralBonusOnFirstPurchase(prisma, fullOrder.id)
          } catch (refErr) {
            console.error("Referral bonus error (non-fatal):", refErr)
          }

          // Referrer points when a referred customer completes a purchase (admin-configured)
