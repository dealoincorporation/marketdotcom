...
export async function GET(request: NextRequest) {
  try {
    ...
    // Calculate total points from rewards (never negative)
    const totalPoints = Math.max(0, rewards.reduce((sum, reward) => sum + reward.points, 0))
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Ensure user.points never goes below 0
    const updatedUser = await prisma.user.findUnique({ where: { id: user.userId }, select: { points: true } })
    if (updatedUser && updatedUser.points < 0) {
      await prisma.user.update({ where: { id: user.userId }, data: { points: 0 } })
    }
